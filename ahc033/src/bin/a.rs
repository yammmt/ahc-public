...
fn main() {
    ...
    // 戦略
    // - とりあえず最初は適当に掃き出す
    //     - 4x4 マスに全部掃き出して大クレーンで確実に動けるようにするか
    //     - E 字状に吐き出して小クレーンの稼働を期待するか
    // - 各クレーン, ターンごとに以下を繰り返す
    //     - 動作が定まっていれば, それに従う
    //         - クレーンが衝突し動けない場合には, 荷物を置いて適当に動く
    //     - 待機中クレーンであれば, ゴールまで運べるものがあれば掴みにいく
    //         - 優先度: クレーン番号が若く, 予定された動きがない順に最短距離優先
    //             - 経路は最短のみを考えても悪くなさそう, 最短を迂回すると最低 2 ターン追加でかかるので
    //     - 待機中クレーンでゴールまで運べるものがなければ, 左端マスを空ける
    //     - 待機中クレーンでゴールまで運べるものも空けるべき左端マスもなければ,
    //       次にゴールまで持っていけるものを狙う
    //     - いずれも動けなければ, 適当に動く
    // - クレーン動作に乱択成分が入るので, ループは時間いっぱい繰り返す
    // - FIXME: 初手で 0, 5, 10, 15, 20 が最奥に固まっていると動けず終わり
    //     - 発生率は 25C5 で 1/50000 よりは小さいはず...だが, 実際は初手以外でも起きるのでもう少し高め
    //     - このときはやむを得ず L -> R 受け流すだけの答えを出す, スコアはとても悪い
    // 必要なもの:
    //     - 盤面の状態: Container(id) or Empty
    //     - 次に運ぶコンテナ
    //     - コンテナの状態: Free/Accepted/BeingMoved/Completed
    //         - 同じコンテナを複数のクレーンが拾いに行くとパスが衝突してしまう
    //     - クレーンの状態: Big/Small と Lift/Empty には座標付き, Removed
    //     - クレーンの予定された行動群: 予定と確定とで分ける
    //         - イレギュラー発生時には上書きされ得る, ターン数さほど多くないので間に合いそう
    //         - 拾いに行く/置きに行くで盤面変わる分考えて再計算をする
    //             - 拾う/置くで経路が変わったり, 拾った時点では次にゴールに持っていけないものが
    //               持っていけるようになっている可能性を考えて
